...
local vault_path = vim.fn.expand "$HOME/Documents/tymon-obsidian/"
...
return {
  {
    ...
    opts = {
      formatters = {
        mdformat = {
          -- Not active in Obsidian vault.
          condition = function(self, ctx)
            -- Get the full path of the current buffer
            local current_file_path = vim.fn.fnamemodify(vim.api.nvim_buf_get_name(0), ":p")

            -- Check if the current file is in the Obsidian vault path
            return vim.fn.match(current_file_path, "^" .. vim.fn.escape(vault_path, "/") .. ".*$") == -1
          end,
        },
      },
    },
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd("FileType", {
        ...
        callback = function()
          ...
          -- TODO: some of these require inputs
          ...
          local mappings = {
            j = {
              ...
              T = { "<cmd>ObsidianTomorrow<CR>", "Tomorrow's Daily note" },
              t = { "<cmd>ObsidianTemplate<CR>", "Template" },
              ["/"] = { "<cmd>ObsidianSearch<CR>", "Search" },
              l = { "<cmd>ObsidianLink<CR>", "Link" },
              L = { "<cmd>ObsidianLinkNew<CR>", "Link New" },
              p = { "<cmd>ObsidianPasteImg<CR>", "Paste Img" },
              r = { "<cmd>ObsidianRename<CR>", "Rename" },
              R = { "<cmd>ObsidianRename --dry-run<CR>", "Rename" },
      ...
      require("obsidian").setup {
        ...
        note_frontmatter_func = function(note)
          ...
          -- local out = { aliases = note.aliases, tags = note.tags, modified = os.date "%A, %B %dth %Y, %I:%M:%S %p" }
